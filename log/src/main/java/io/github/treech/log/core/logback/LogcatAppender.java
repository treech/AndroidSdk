        ...
        switch (event.getLevel().levelInt) {
            ...
            case Level.TRACE_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.VERBOSE)) {
                    log(Log.VERBOSE, tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.DEBUG_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.DEBUG)) {
                    log(Log.DEBUG, tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.INFO_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.INFO)) {
                    log(Log.INFO, tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.WARN_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.WARN)) {
                    log(Log.WARN, tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.ERROR_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.ERROR)) {
                    log(Log.ERROR, tag, this.encoder.getLayout().doLayout(event));
                }
                break;
